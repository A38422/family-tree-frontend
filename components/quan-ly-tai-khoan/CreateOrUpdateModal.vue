<template>
  ...
  >
    ...
    >
      ...
      <FormItem v-if="!isUpdate" label="Xác nhận mật khẩu" prop="rePassword">
        <Input
...
export default {
  ...
  computed: {
    ...
    rules() {
      return {
        ...
        role: [
          { required: true, validator: this.validateRole, trigger: 'blur' },
        ],
        member: [
          { required: true, validator: this.validateMember, trigger: 'blur' },
        ],
      }
    },
  ...
  methods: {
    ...
    validateMember(rule, value, callback) {
      if (!value) {
        return callback(new Error('Vui lòng chọn thành viên để liên kết tài khoản'))
      }
      callback()
    },
  },
