<template>
  <div class="list-bao-cao">
    ...
    <div class="custom-navigate">
      ...
      <Button type="primary" :loading="loadingExport" @click="handleExport">
    ...
    handleExport() {
      ...
      const title =
        this.timeRange && this.timeRange.length > 0
          ? `Báo cáo ${
              this.reportType === 'thu-chi' ? 'thu chi' : this.reportType
      ...
      for (let i = 0; i < body.getElementsByTagName('tr').length; i++) {
        body
          ...
          [i].removeChild(
            body.getElementsByTagName('tr')[i].getElementsByTagName('td')[
              body.getElementsByTagName('tr')[i].getElementsByTagName('td')
                .length - 1
            ]
          )
        ...
        .then((pdf) => {
          ...
          for (let i = 1; i <= totalPages; i++) {
            ...
            if (i === totalPages) {
              ...
              const textWidth =
                pdf.getStringUnitWidth(totalAmount) * pdf.internal.getFontSize()
